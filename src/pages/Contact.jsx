...
function Contact() {
  ...
  const handleSubmit = async (e) => {
    ...
    // 5. Rate Limiting (1 submission per minute per user)
    const lastSubmit = localStorage.getItem('tridex_contact_submit_time');
    const now = Date.now();
    if (lastSubmit && now - parseInt(lastSubmit) < 60000) {
      const waitTime = Math.ceil((60000 - (now - parseInt(lastSubmit))) / 1000);
      setError(`You have already submitted an enquiry. Please wait ${waitTime} seconds before submitting again.`);
      return;
    }
